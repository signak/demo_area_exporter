...
import '../../../core/exceptions/map_tile_db_exception.dart';
import '../../../core/utils/logger.dart';
import '../../domain/repositories/map_tile_record.dart';
import '../../domain/repositories/map_tile_repository.dart';
import 'firestore_map_tile_record.dart';
import 'fs_constants.dart';

class FirestoreMapTileRepository extends MapTileRepository {
  FirestoreMapTileRepository();

  final _db = FirebaseFirestore.instance;

  @override
  MapTileRecord getInfoForFailSafe() {
    return FirestoreMapTileRecord(
      id: 0,
      tileIndex: 0,
  ...
  List<Map<String, dynamic>> get _initialRecords {
    final List<Map<String, dynamic>> records = [
      ...
      {
        FsMapTiles.id: 4,
        FsMapTiles.tileIndex: 4,
        FsMapTiles.name: '国土地理院（シームレス写真）',
        FsMapTiles.tileUri:
  ...
  Stream<List<MapTileRecord>> watchTiles({
    ...
    final queryWithConverter = query.withConverter<MapTileRecord>(
      fromFirestore: (snapshot, _) =>
          FirestoreMapTileRecord.fromJson(snapshot.data()!),
      toFirestore: (value, options) =>
          (value as FirestoreMapTileRecord).toJson(),
    );

    return queryWithConverter.snapshots().transform<List<MapTileRecord>>(
          StreamTransformer.fromHandlers(
            handleData: (data, sink) {
              logger.d('[FsMapTileInfoRepo] handleData size=${data.size}');
              final ret = <MapTileRecord>[];
  ...
  void _handleError(
    ...
    logger.e(
      '[FsMapTileInfoRepo] handleError',
      error: ex,
      stackTrace: ex.mergedStackTrace,
    );
    sink.addError(ex);
  }
}
